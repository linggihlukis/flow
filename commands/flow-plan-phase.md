...
6. **Research depth auto-calibration** — if `.flow/codebase/patterns.md` exists:
   ...
   b. For each zone name, run an awk-scoped grep against ONLY the `## Confidence Notes`
      section of PATTERNS.md — do NOT grep the full file (prevents false matches from
      zone names appearing in other sections):

      ```bash
      awk '/^## Confidence Notes/{f=1;next} /^## [^#]/{f=0} f' .flow/codebase/patterns.md \
        | grep -qi "[zone_name]"
      ```
...
7. **Zone-scoped PATTERNS.md extraction** — if `.flow/codebase/patterns.md` exists:
   ...
   b. Create `M/phases/phase-$ARGUMENTS/patterns-scope.md` containing:
      ...
      Extract each section using awk — from its `##` header to the next `##` header:
      ```bash
      awk '/^## Do Not Change/{f=1} f; /^## [^#]/ && f && !/^## Do Not Change/{f=0}' \
        .flow/codebase/patterns.md
      ```
      Repeat for each global and zone-specific section. Concatenate all extracted
      sections into `patterns-scope.md`.
...
8. **Repo-map generation (optional)** — check if `[flow-tools-path]` exists:

   a. If available:
      Extract the zone list from `M/phases/phase-$ARGUMENTS/CONTEXT.md`
      (same list used in step 7).

      Run the indexer with phase scope:
      ```bash
      node [flow-tools-path] index \
        --scope [zone1] [zone2] \
        --phase $ARGUMENTS \
        --patterns .flow/codebase/patterns.md \
        --cwd .
      ```
   ...
   b. If `[flow-tools-path]` is not available:
      ...
      ```bash
      node [flow-pkg-dir]/bin/flow-tools.js index \
        --scope [zone1] [zone2] \
        --phase $ARGUMENTS \
        --patterns .flow/codebase/patterns.md \
        --cwd .
   ...
   - Include the repo-map path in the researcher brief (Stage 1):
     ```
     Repo-map: M/phases/phase-$ARGUMENTS/repo-map.json
     ```
   - Read `treesitter_health` from `M/phases/phase-$ARGUMENTS/repo-map.json`
     and print one diagnostic line:
     ```
     ✓ treesitter: wasm=[true/false]  parsed=[N]  errors=[N]  includes=[N]  ast_yield=[N]
     ```
   - If `wasm_loaded` is `false`:
     ```
     ⚠️  treesitter WASM did not load — repo-map entries will be stubs. Researcher proceeds without AST data.
     ```
     Proceed regardless. The indexer is optional.

   If neither is available or all fail — skip silently. Researcher proceeds
   without repo-map (existing behavior).
...
- Identify files the researcher will receive: CONTEXT.md, patterns-scope.md (or
  PATTERNS.md fallback), requirements.md, SERVICE-MAP.md (if cross-service phase)
- For each file, get size: `wc -c [file]` (Windows: `(Get-Content [file] -Raw).Length`)
- Calculate: `sum_of_all_chars ÷ 4`, round to nearest 100
- If `M/phases/phase-$ARGUMENTS/context-log.md` does not exist, create it with
  the table header (see AGENTS.md §21)
- Append one row: `| [now ISO 8601] | flow-researcher | [est_tokens] | [file list] |`
...
**Context-size advisory (A1):** After the budget check completes (or is skipped),
evaluate the researcher's estimated token load relative to the model context limit:

1. Read `config.json` → `context.model_context_limit`. If absent → skip.
2. Use the same token estimation from the trace entry above
   (`sum_of_all_chars ÷ 4`, rounded to nearest 100).
...
Repo-map: M/phases/phase-$ARGUMENTS/repo-map.json (if exists — use for file/function discovery before reading source files)
...
Extract the researcher's `## Return` block from `M/phases/phase-$ARGUMENTS/research.md`. Use `approach_summary`, `critical_gotchas`, and `open_questions` fields to inform the planner brief. If `status: blocked` — stop and surface the open_questions to the developer before continuing.

Read researcher Return block → `patterns_stale`.

If `patterns_stale` is empty or absent:
  Proceed to spawn @flow-planner normally.

If `patterns_stale` is non-empty:

  Read `.flow/state.md` → status.

  If status indicates execution is in progress (i.e. not "planned", "ready",
  "not-started", "verified", "milestone-complete", or "complete" — tasks have
  already been executed in the current phase):
    ...
    "⚠️ Staleness detected in [zone(s)]: [patterns_stale entries].
     ...
     Run /flow-map-codebase --refresh after this phase completes to
     incorporate the correction permanently into PATTERNS.md."
    Proceed to spawn @flow-planner with the corrected research.md.
  ...
  If status is "planned" or no tasks have been executed yet (between phases):
    ...
    "⚠️ Staleness detected in [zone(s)] before planning began:
     ...
     Options:
       ...
       (b) Pause — run /flow-map-codebase --refresh now, then re-run
           /flow-plan-phase to start with a corrected PATTERNS.md."
    ...
    If developer chooses (b):
      ...
      ```
      Then stop. When the developer re-runs /flow-plan-phase $ARGUMENTS after
      the refresh, the Pre-flight step 0 gate will detect the sentinel and
      route directly to the recovery section.

---

## Research Completeness Gate

Before spawning the planner, verify that research.md provides sufficient evidence
to plan from. This gate is model-agnostic — it checks for the presence of evidence,
not the quality of reasoning.

Read `M/phases/phase-$ARGUMENTS/CONTEXT.md` and
`M/phases/phase-$ARGUMENTS/research.md` together.
...
For each row in the Evidence Summary table:
  □ Is the Locked Decision column non-empty and matches a locked decision
    in CONTEXT.md?
  □ Is File Path(s) a real file path (not "N/A" or empty)?
  □ Is Verbatim Anchor a specific code line or "new file — no existing anchor"?
    Rows with "N/A" in Verbatim Anchor indicate a blocked decision — surface
    these to the developer before spawning the planner.
...
If research.md does NOT contain a `## Evidence Summary` section, fall back to the
original prose-check:

**Check 1 — File coverage:**
For each file named in CONTEXT.md's scope (under "What This Phase Does" or equivalent
scope section):
  □ Is the exact file path named in research.md — not paraphrased, not just its
    directory or module?
  □ Is there at least one grep-confirmed finding for that file (not just a
    description of what the file contains)?
  □ Is there a concrete insertion point — the actual surrounding lines or function
    signature, not "locate the X section"?

**Check 2 — Locked decision coverage:**
For each locked decision in CONTEXT.md's "Locked Decisions" table (or equivalent):
  □ Does research.md have a corresponding finding that shows how to implement
...
**If any box is unchecked:**
  ...
  which files or decisions lack evidence. The brief must name:
    - The specific file path(s) missing grep-confirmed findings
    - The specific locked decision(s) missing a corresponding codebase finding
  Instruct the researcher to append findings to research.md — not rewrite it.

  Repeat the completeness check after re-investigation.
  Maximum 2 re-investigation rounds.
