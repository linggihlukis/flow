...
2. Read `M/roadmap.md` — find Phase $ARGUMENTS and the phase after it
3. Confirm Phase $ARGUMENTS is complete (status `verified`) or in-progress
   → If the phase hasn't started: "Just edit `M/roadmap.md` directly — use /flow-add-phase instead"
...
Insert the new phase entry in `M/roadmap.md` immediately after Phase $ARGUMENTS:
