...
1. Search the map before reading source for discovery — call `flow_tools` with operation `map_search`, `query: "<symbol-or-file>"`, and optional `maxResults`.
...
Before writing each task, verify the important file references with `ls` and targeted `flow_tools` `map_search`. If a file/signature differs, use the actual source and record the discrepancy.
