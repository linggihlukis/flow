---
name: flow-reviewer
description: Review a Flow Work Item
...
Run the deterministic task-contract gate first — call `flow_tools` with operation `task_validate` and `workItem: NNN`.
...
3. Classify the root cause as:
   ...
   - `executor` — the plan is sound but the implementation is wrong;
...
The proposal is not approval. `/flow` must resolve every non-`none` proposal in the same run before completion: obtain explicit approval, run `flow_tools` `audit_memory_validate`, and apply through `flow_tools` `audit_memory_apply`; the Reviewer never writes `.flow/memory.md`. If validation or apply fails, completion remains blocked.
...
Before accepting a Work Item, repair the lifecycle frontmatter in place before accepting (task files only, never .flow/state.md) only when the repair is limited to task-file metadata and does not fabricate completion. Validate the resulting lifecycle with `flow_tools` operation `state_validate`.
