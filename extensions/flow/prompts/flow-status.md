...
Call `flow_tools` for each read-only operation:

- `state_get` — active_work_item, status, updated_at, git_commit
- `state_validate` — frontmatter + required fields
- `audit_open` — `.flow` integrity (state.md + work-items/ + map.json)
- `files_check` with `paths: [".flow/memory.md"]` and `lineCount: true` — size
...
Run at session start, before planning, or whenever you need orientation. Replaces `flow-progress` (minus milestone/phase table) and `flow-health` workflow surface — health checks are now the `flow_tools` operations `audit_open`, `state_validate`, and `state_sync`.
